...
\footnotetext{%
	Unhelpfully, we now have two meanings of \emph{transitive}; one for equivalence relations and one for actions.%
}
...
% \footnotetext{%
% 	The two are equivalent: if $y$ has order $p$, then $\ip{y}$ is a subgroup of order $p$. If $H\le G$ has order $p$, then $H\cong \Z_p$ is cyclic.%
% }
