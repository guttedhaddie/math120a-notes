...
\begin{exercises}{}{}
	...
	\begin{enumerate}    
	  ...
	  \item Recall Example \ref{ex:conjexample}.1. Find another element $\nu\neq\rho$ for which $\nu\sigma\nu^{-1}=\tau$. Now determine \emph{how many} distinct such $\nu$ exist: that is, how many ways can we conjugate $\sigma$ to get $\tau$?
	  ...
	  is an equivalence relation on any group $G$. 
		...
		\item\label{exs:conjsubgroup} Let $H\le G$, fix $a\in G$ and define the \emph{conjugate subgroup} $K=c_a(H)=\{aha^{-1}:h\in H\}$.
	  \begin{enumerate}
	...
	\end{enumerate}
	  ...
	  \item Let $G$ be a group. Prove directly that $Z(G)\triangleleft G$, \emph{without} using Theorem \ref{thm:centernormal}. That is:
		...
		\item\label{exs:centralizerSn} The \emph{centralizer} of an element $x\in G$ is the set of elements which conjugate $x$ to itself:
		\[
			C(x)=\big\{g\in G:gx g^{-1}=x\big\}
		\]
		\begin{enumerate}
		  \item Prove directly that $C(x)$ is a subgroup of $G$.
		  \item We compute a few centralizers directly. The centralizer of $\sigma=(1\,2\,3)\in S_3$ is the set of all $\rho$ for which
			\[
				(1\,2\,3)=\rho\sigma\rho^{-1} =\big(\rho(1)\,\rho(2)\,\rho(3)\big)
			\]
			Otherwise said, $\rho$ must permute the elements $\{1,2,3\}$ \emph{in order.} We conclude that 
			\[
				C(\sigma)=\big\{e,(1\,2\,3),(1\,3\,2)\big\}=\{e,\sigma,\sigma^2\}\cong\Z_3
			\]
			In similar fashion, compute the centralizers of the following elements and identify the group $C(\sigma)$:
			\begin{enumerate}
			  \item $\sigma=(1\,2)\in S_4$
			  \hspace{4cm}
			  ii. \ $\sigma=(1\,2\,3\,4\,5)$ in both $S_5$ and $A_5$.
			  \setcounter{enumiii}{2}
			  \item $\sigma=(1\,2)(3\,4)$ in both $S_4$ and $A_4$.
			\end{enumerate}
		\end{enumerate}
		
		
		\item\label{exs:centerquotientcyclic} Prove that if $\quotient G{Z(G)}$ is cyclic, then $G$ is abelian. What is the group $\quotient G{Z(G)}$ in such a situation?
