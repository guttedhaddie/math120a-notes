...
When $H$ is not a subgroup, the coset construction is unlikely to produce a partition.% (though see Exercise \ref{exs:partitioncoset}).
...
\begin{exercises}{}{}
	...
	\begin{enumerate}
	  \item Find the cosets of the following subgroups: since the groups are abelian, left and right cosets are identical.
		\begin{enumerate}
	  	\item \makebox[200pt][l]{$4\Z\le 2\Z$\hfill (b)\lstsp} $\ip{4}\le\Z_{10}$
		  \item[(c)] \makebox[200pt][l]{$\ip{6}\le\Z_{30}$\hfill (d)\lstsp} $\ip{20}\le\Z_{30}$
		\end{enumerate}
...
\footnotetext{This is sometimes misremembered as `the order of an element divides the order of the group.' This is the special case when $H$ is a \emph{cyclic subgroup} of $G$. The even more special case when $G$ is cyclic is Corollary \ref{cor:subscyclic}:
$\ip s\le\Z_n$ has order $\frac n{\gcd(s,n)}$ (certainly divides $n$). The converse to Lagrange is \emph{false}: e.g.{} $A_4$ has order 12, but no subgroup of order 6 (Exercise \ref*{sec:transpositions}.\ref{exs:a4subgroups}).}
...
We could similarly have proved this using the right coset partition. %Revisit the proof of Theorem \ref{thm:alt} ($\nm{A_n}=\frac 12\nm{S_n}$): it is nothing but a special case of Theorem \ref{thm:cosetpart} and the above argument.\smallbreak
...
%Generally, let $\mathcal L=\{gH:g\in G\}$ and $\mathcal R=\{Hg:g\in G\}$ be the sets of left and right cosets of $H$ in $G$. By the Axiom of Choice, there exists a choice function $f:\mathcal L\to G$ such that $f(gH)\in gH$ for each left coset $gH$. Now left $I=\range(f)\subseteq G$. Clearly $f:\mathcal L\to I$ is a bijection.\\
% 	Now suppose that $g_1,g_2\in I$ and observe that
% 	\[Hg_1=Hg_2\iff g_2g_1^{-1}\in H\iff g_1^{-1}H=g_2^{-1}H\]
% 	It follows that $J=\{g\in G:g^{-1}\in I\}$ enumerates the right cosets: there is a bijection $\hat f:\mathcal R\to J$. We conclude that $\hat f^{-1}\circ f:\mathcal L\to\mathcal R$ is a bijection, whence the cardinality of the sets of left and right cosets are equal.
...
\end{proof}
% 
% \begin{proof}[Proof (long version)]
% If $(G:H)$ and $(H:K)$ are finite, then they are necessarily natural numbers. Thus let $p=(G:H)$ and $q=(H:K)$ where $p,q\in\N$. We may therefore enumerate the left cosets of $H$ in $G$: $\exists$ distinct $g_i\in G$ such that these are
% \[g_1H,g_2H,\ldots,g_pH.\]
% We may also enumerate the left cosets of $K$ in $H$: $\exists$ distinct $h_j\in H$ such that these cosets are
% \[h_1K,h_2K,\ldots,h_qK.\]
% We claim that the left cosets of $K$ in $G$ are given by $(g_ih_j)K$ where $i=1,\ldots,p$ and $j=1,\ldots, q$. Essentially
% \[\begin{array}{ccccc}
% g_1h_1K&g_1h_2K&\cdots&g_1h_qK\\
% g_2h_1K&g_2h_2K&\cdots&g_2h_qK\\
% \vdots&\vdots&\ddots&\vdots\\
% g_ph_1K&g_ph_2K&\cdots&g_ph_qK\\
% \end{array}\]
% If our claim is correct, then there are clearly $pq$ left cosets of $K$ in $G$.\\
% To prove the claim, first note that each set $g_ih_jK$ \emph{is} a left coset of $K$ in $G$. If we can show that the above list of cosets \emph{partitions} $G$, then we know that these are \emph{all} of the required left cosets. We therefore need to see that every element of $G$ lies in exactly one of these cosets.
% \begin{enumerate}
%   \item Let $x\in G$ be given. Since the left cosets of $H$ partition $G$ we see that every $x$ lies in exactly one left coset $x\in g_iH$. Therefore $g_i^{-1}x\in H$. However every element of $H$ lies in exactly one left coset $h_jK$. It follows that $\exists i,j$ such that $x\in (g_ih_j)K$. Every $x\in G$ is therefore in one of the claimed cosets of $K$.
%   \item Now suppose $y\in g_ih_jK\cap g_\alpha h_\beta K$ for some $i,j,\alpha,\beta$. Then $y\in g_iH\cap g_\alpha H\implies \alpha=i$, since the left cosets of $H$ partition $G$. But then $g_i^{-1}x\in h_jK\cap h_\beta K\implies \beta=j$ since the left cosets of $K$ in $H$ partition $H$. It follows that the cosets $(g_ih_j)K$ are disjoint.
% \end{enumerate}
% By 1 and 2, the left cosets $(g_ih_j)K$ partition $G$. It follows that these are precisely the left cosets of $K$ in $G$, and the result follows.
% \end{proof}
...
\begin{examples*}{\ref{ex:cosetexs}.1 cont}{}
...
\begin{enumerate}\setcounter{enumi}{1}
  \item[]\begin{enumeratea}\itemsep0pt
		\item \emph{Choose representatives}: Make a choice of elements $a$ and $b$ in the respective cosets.
		\item \emph{Add within the original group}: Compute $a+b\in\Z_{12}$.
		\item \emph{Take the coset}: Return the left coset $(a+b)+H$.
	\end{enumeratea}
...
\footnotetext{That these arguments are converses is typical: for a given function $\mu:A\to B$,
\begin{itemize}
  \item Well-definition means: $a=b\implies \mu(a)=\mu(b)$
  \item Injectivity means: $\mu(a)=\mu(b)\implies a=b$
\end{itemize}}
...
For $G=\Z_4\times\Z_8$ and three subgroups $H$, we identify the factor group $\smash{\quotient GH}$. %The examples start easy and become progressively more difficult, and all are long, so take your time!
...
% 	 How about obtaining an explicit isomorphism $\quotient{(\Z_4\times\Z_8)}{\ip{(0,1)}}\cong\Z_4$? Given that the above coset is a generator, simply map it to a generator of $\Z_4$: indeed
...
%   homomorphism and appeal to the first isomorphism theorem.
...
\boldsubsubsection{Other Examples}
...
\begin{enumerate}
  ...
  is a well-defined homomorphism (mimic the proof of Theorem \ref{thm:znisomdefn}!).
	...
	\item Let $G$ be any group. To what groups are \smash{$\quotient G{\{e\}}$ and $\quotient GG$} isomorphic?
