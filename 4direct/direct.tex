...
\begin{quote}
\scalebox{1}{$\begin{array}{c||c|c|c|c}
...
\footnotetext{In this course we will only ever have \emph{finitely many} terms in a direct product/sum: in such cases these concepts are identical for abelian groups written additively. When there are infinitely many factors, the concepts are slightly different.}
...
\boldsubsubsection{Finite(ly generated) abelian groups}
...
\begin{enumeratea}\itemsep0pt
  \item $\Z_2\times\Z_{3^2}\times\Z_{5^2}\cong \Z_{450}$\hfill (cyclic, max order 450)
...
We finish by listing all groups of orders 1 through 15 and abelian groups of order 16 up to isomorphism. The Fundamental Theorem gives us all the abelian groups.\par
