...
In this chapter we partition a group into subsets in such a way that the \emph{set of subsets} inherits a natural group structure. This is a long and abstract story, though the essentials aren't new; this is precisely what happens with modular arithmetic (itself a generalization of even and odd).
...
Our main goal is to generalize Example \ref{ex:cosetsimple}. Start by observing that the identity element $[0]\in\Z_3$ is in fact a \emph{subgroup} ($3\Z$) of $\Z$ from which the other subsets $[1]$, $[2]$ are obtained by \emph{translation.}
...
\begin{example*}{\ref{ex:cosetsimple} cont}{}
	Since $G=\Z$ is written additively, the left and right cosets of $H=[0]=3\Z$ containing $g$ are written
	\[
		g+H:=\{g+h:h\in H\}
		\qquad\qquad 
		H+g:=\{h+g:h\in H\}
	\]
	These cosets are precisely the elements of $\Z_3$!
	\begin{align*}
	...
	\begin{enumerate}
	  \item Find the cosets of each subgroup: since the groups are abelian, left and right cosets are equal.
		...
		\item\label{exs:va4}\begin{enumerate}
		  ...
		  \item Repeat the question for the subgroup $V:=\{e,(1\,2)(3\,4),(1\,3)(2\,4),(1\,4)(2\,3)\}$ of $A_4$.
		\end{enumerate}
		
	
		\item\label{exs:d4normal} Revisit Examples \ref{ex:disym}, particularly its use of cycle notation.
		\begin{enumerate}
		  \item Find the left and right cosets of $H=\{\rho_0,\delta_1\}\le D_4$. Is $H$ normal?
...
\begin{proof}
	...
	\]
	is a bijection (inverse function $(L_g)^{-1}=L_g^{-1}$). Every left coset of $H$ therefore has the same cardinality as $H$. Since the left cosets partition $G$ (Theorem \ref{thm:cosetpart}), we conclude that
	\[
...
\begin{proof}
	...
	\end{itemize}
	We conclude that $\nm{\ip g}=p$. But then $G=\ip{g}$ is cyclic and so isomorphic to $\Z_p$ (Theorem \ref{thm:cyclicisomorph}).
\end{proof}
...
\begin{examples}{}{indexeasy}
	\exstart If $G=\Z_{20}$ and $H=\ip{2}=\{0,2,4,\ldots,18\}$, then there are $(G:H)=\frac{20}{10}=\frac{\nm G}{\nm H}=2$ cosets (left and right are equal here):
	\[
		...
		\]
	\begin{enumerate}\setcounter{enumi}{1}
	  \item\label{ex:indexeasy2} Recall the orthogonal and special orthogonal groups (Example \ref{ex:orthogonal} \& Exercise \ref*{sec:subgroup}.\ref{exs:subgpmatrix}):
	  \[
	  ...
	  \begin{align*}
	  	...
	  	&\iff \det B=\det A
	  \end{align*}
	  Since determinant can take only two possible values in $\rO_n(\R)$, we conclude that there are precisely two cosets $\bigl(\rO_n(\R):\rSO_n(\R)\bigr)=2$.
	  \item\label{ex:ZsubgroupRindex} The integers form an (additive) subgroup of the real numbers $\R$. Observe that
	  \[
	  	x\Z=y\Z\iff y-x\in \Z
	  \]
	  It follows that there is exactly one coset for every real number in the half-open interval $[0,1)$; otherwise said, $\psi(x)=x\Z$ defines a bijection of the interval $[0,1)$ with the set of cosets $\{x\Z\}$. We conclude that there are uncountably many cosets!
	\end{enumerate}
\end{examples}
...
\begin{example*}{\ref{ex:indexeasy}.1, cont.}
	If $G=\Z_{20}$, $H=\ip{2}$ and $K=\ip{10}$, then
	\[
		K=\{0,10\}
	...
	\begin{itemize}
		\item $(G:H)=2$ with cosets $H$ and $\textcolor{red}{1}+H$. Each coset has the form $\textcolor{red}{g_i}+H$ with either $\textcolor{red}{g_0=0}$ or $\textcolor{red}{g_1=1}$: these values are \emph{representatives} of the two cosets.
		...
		\]
		\item $(G:K)=\frac{20}{2}=10=(G:H)(H:K)$. The cosets this time are
		\[
			K=\{0,10\},\quad 
			1+K=\{1,11\},\quad 
			2+K=\{2,12\},\quad 
			...
			9+K=\{9,19\}
		\]
		with representatives $0,1,\ldots,9$. The crucial observation for the proof is that representatives are formed by \emph{summing} those above: the cosets of $K$ in $G$ are precisely $(\textcolor{red}{g_i}+\textcolor{blue}{h_j})+K$.
	\end{itemize}
\end{example*}
...
\begin{proof}
	...
	\[
		...
		(H:K)=\nm{\{\textcolor{blue}{h_j}\}}
	\]
	We claim that the left cosets of $K$ in $G$ are precisely the sets $(\textcolor{red}{g_i}\textcolor{blue}{h_j})K$. Certainly each of these is a coset; we show that these \emph{partition} $G$, whence the collection $\{(\textcolor{red}{g_i}\textcolor{blue}{h_j})K\}$ must comprise \emph{all} left cosets.
	\begin{itemize}
	  \item Every $g\in G$ lies in some left coset of $H$, so $\exists \textcolor{red}{g_i}\in G$ with $g\in \textcolor{red}{g_i}H$.\par
	  Now $\textcolor{red}{g_i}^{-1}g\in H$ lies in some left coset of $K$ in $H$, so $\exists \textcolor{blue}{h_j}\in H$ with $\textcolor{red}{g_i}^{-1}g\in \textcolor{blue}{h_j}K$.\par
	  But then $g\in (\textcolor{red}{g_i}\textcolor{blue}{h_j})K$, whence every $g\in G$ lies in at least one set $(\textcolor{red}{g_i}\textcolor{blue}{h_j})K$.
...
\begin{exercises}
	...
	\begin{enumerate}
	  \item Find the indices of the following subgroups:
		...
		\item Let $G=\Z_8$, $H=\ip 2=\{0,2,4,6\}$ and $K=\ip 4=\{0,4\}$. Write out all the cosets for the three subgroup relations $K\le H$, $H\le G$ and $K\le G$, and verify the index multiplication formula (Theorem \ref{thm:tower}).
		
		
		\item Let $G=S_4$ and consider the subgroup tower $K\le H\le G$ where
		\[
		...
		\]
		\begin{enumerate}
		  \item Write out the elements of the two left cosets $K\le H$, namely $K=\textcolor{blue}{e}K$ and $\textcolor{blue}{(1\,2)}K$ with representatives $\textcolor{blue}{h_0=e}$ and $\textcolor{blue}{h_1=(1\,2)}$.
		  \item Repeat part (a) for the four cosets of $H$ in $G=S_4$: observe that $\textcolor{red}{g_0=e}$, $\textcolor{red}{g_1=(1\,4)}$, $\textcolor{red}{g_2=(2\,4)}$, $\textcolor{red}{g_3=(3\,4)}$ are representatives.
		  \item Compute the eight left cosets of $K$ in $S_4$ and verify that they are $(\textcolor{red}{g_i}\textcolor{blue}{h_j})K$ in accordance with the proof of Theorem \ref{thm:tower}.
		\end{enumerate}
	  ...
	  \]
	  is injective from the set of left cosets to the set of right cosets.\par
	  ...
	  \item\label{exs:zqindex} We modify Example \ref{ex:indexeasy}.\ref{ex:ZsubgroupRindex}.
	  \begin{enumerate}
	    \item The sets $\Q$ and $\Z$ are both groups under addition. Show that there is exactly one coset of $\Z$ in $\Q$ for each rational $q\in[0,1)$. Hence conclude that $(\Q:\Z)=\aleph_0$ is countably infinite.
	    \item Describe the index of $U_n$ ($n\th$ roots of unity) as a subgroup of the circle group $S^1$.		
% 	  \item The group $U_n$ is a subgroup of the circle group $S^1$. Observe that
% 	  \[
% 	  	e^{i\theta}U_n=e^{i\psi}U_n\iff e^{i(\theta-\psi)}\in U_n\iff e^{i(\theta-\psi)}=\zeta_n^k
% 	  \]
% 	  for some $k\in\{0,\ldots,n-1\}$. Since $\zeta=e^{\frac{2\pi i}{n}}$
		\end{enumerate}
	\end{enumerate}
\end{exercises}
...
\end{enumerate}
If $\quotient GH$ is abelian, the Fundamental Theorem (\ref{thm:fund}) might supply candidates. Step 2 can often be accomplished by considering orders of elements (cosets). The simple observation can help with this.

\begin{lemm}{}{ordercoset}
	Let $\quotient GH$ be a factor group. Then $(gH)^m=H\Longleftrightarrow g^m\in H$\quad ($mg\in H$ if $G$ additive).\par
	By Corollary \ref{cor:orderdefn}, the \textbf{order of the element} $gH\in \quotient GH$ is the smallest $m\in\N$ for which $g^m\in H$.\par
	Moreover, this number $\min\{m\in\N:g^m\in H\}$ divides the order of $g$ (in $G$).
\end{lemm}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \begin{minipage}[t]{0.8\linewidth}\vspace{-18pt}
	  	\[
				\begin{array}{@{}ll}
		  		...
		  		\text{Reflections:}&\mu R_n=\bigl\{\mu_1,\ldots,\mu_n\bigr\}, \text{ where $\mu$ is any reflection.}
				\end{array}
		  \]
	  \end{minipage}
	  \hfill
	  \begin{minipage}[t]{0.19\linewidth}\vspace{-20pt}
	  	\flushright$\begin{array}{c||cc@{}}
	  		\circ&R_n&\mu R_n\\\hline\hline
	  		R_n&R_n&\mu R_n\\
	  		\mu R_n&\mu R_n&R_n
	  	\end{array}$
	  \end{minipage}\smallbreak
		The Cayley table for the factor group $\quotient{D_n}{R_n}=\{R_n,\mu R_n\}$ is shown. Consider how the calculation $(\mu R_n)(\mu R_n)=R_n$ says that the composition of two reflections is a rotation!
...
% 	  is a well-defined isomorphism. The factor group construction can be visualized as wrapping the real line infinitely many times around a circle of radius 1 (circumference $2\pi$).
	  ...
	  \item\label{ex:projectvectspace} (Example \ref*{ex:cosetexs}.\ref{ex:cosetsubspace}, simplified) \ Given $W=\{(0,y):y\in\R\}\le \R^2$, each coset (vertical line)
	  \[
	  ...
	  \item\label{ex:hardfactorgroupcount} (Hard) \ Let $G=\Z\times\Z_4$ and consider the subgroup
	  \[
...
We'll see more examples, and revisit others, in Section \ref{sec:1stiso} once we've developed more machinery.
...
\begin{exercises}
	...
	\begin{enumerate}
	  \item List the cosets of the subgroup $H=\ip{3}$ in $G=\Z_{15}$. By mimicking the proof of Theorem \ref{thm:znisomdefn}, show that $\psi:\Z_3\mapsto \quotient GH:x\mapsto x+H$ is a well-defined isomorphism.
% 	  \[
% 	  	\psi:\Z_3\mapsto \quotient GH:x\mapsto x+H
% 	  \]
	
		
		\item (Exercise \ref*{sec:cosetnormal}.\ref{exs:VfactorV} cont.) \ If $H=\bigl\{(0,0),(0,2),(2,0),(2,2)\bigr\}$, identify the factor group $\quotient{\Z_4\times\Z_4}H$.
		...
		\item Let $G=\Z_9\times\Z_9$.
		\begin{enumerate}
		  \item State the elements of the subgroup $H=\ip{(3,6)}$. Now identify $\quotient GH$ by showing that every element of the factor group has order at most 9 and that it contains an element of order 9.
...
% 	
% 	 	\item* HARD (See Examples \ref{ex:factorfiniteabelian})\lstsp Let $G=\Z_4\times\Z_8$. Prove that each $\psi$ is a well-defined isomorphism:
...
% 		(\emph{Bijectivity follows from the description of the cosets, though proving injectivity might be instructive.})
		...
		\item (Exercise \ref*{sec:lagrange}.\ref{exs:zsqrt2subgroup}, cont.) \ The factor group $\quotient GH$ is abelian and of order 6, whence it is cyclic. Prove this explicitly by finding a generator and thus an isomorphism $\psi:\Z_6\to\quotient GH$.
		
	
	
		\item (Example \ref*{ex:normalmoreex}.\ref{ex:a4normal}, cont.) \ Find an explicit isomorphism $\psi:\Z_3\cong\quotient{A_4}{V}$.
		...
		\item If $H\triangleleft G$ has index $(G:H)=2$, then the factor group $\quotient GH$ is necessarily isomorphic to $\Z_2$.
		\begin{enumerate}
		  \item Prove that $\psi:\Z_2\to\quotient{S_n}{A_n}$ defined by $\psi(x)=(1\,2)^xA_n$ is an isomorphism.
		  \item Find an explicit isomorphism $\psi:\Z_2\to \quotient{\rO_n(\R)}{\rSO_n(\R)}$.
		\end{enumerate}
		
		
		\item\label{exs:normaldirectfactor} (Exercise \ref*{sec:cosetnormal}.\ref{exs:directprodfactor}, cont.)\lstsp Suppose $G=H\times K$, $J\triangleleft H$ and $\widehat J=J\times\{e_K\}$. Prove that $\quotient G{\widetilde J}=\quotient HJ\times K$.\par
		Can you quickly verify Examples \ref{ex:factorfiniteabelian}.1, 2, and \ref*{ex:normalmoreex}.\ref{ex:projectvectspace} in this language?
		
		
		\item\label{exs:ordercoset} Complete the proof of Lemma \ref{lemm:ordercoset} by showing that the order of the element/coset $gH\in\quotient GH$ divides the order of the element $g\in G$.\par
		(\emph{Hint: Use the division algorithm similarly to the proof of Theorem \ref{thm:cyclic1}})
