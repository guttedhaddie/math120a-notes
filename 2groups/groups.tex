...
\begin{exercises}
	...
	\begin{enumerate}
	  ...
	  \item Briefly explain why ``subgroup'' is transitive: that is, if $K\le H$ and $H\le G$, then $K\le G$.
		...
		\item\label{exs:subgroupalt} Let $H$ be a non-empty subset of a group $G$. Prove that $H$ is a subgroup of $G$ if and only if
	  \[
...
\begin{exercises}
	...
	\begin{enumerate}
	  ...
	  \begin{enumerate}
	    \item\label{exs:subgpmatrix1} Special linear group: $\rSL_n(\R)=\bigl\{A\in \rM_n(\R):\det A=1\bigr\}$
...
% 	    \!\right)$}$ is a block matrix and $I_n$ the $n\times n$ identity matrix.
	  ...
	  \begin{minipage}[t]{0.69\linewidth}\vspace{0pt}
	    ...
	    In case you don't recognize it, the pictured \textcolor{red}{red die} is not one of the five Platonic solids: it has ten kite-shaped faces, and its rotation group has order 10. We'll return to these examples in later sections.
...
\begin{exercises}{}
	...
	\begin{enumerate}	  
		\item\label{exs:easyhomoex} Which of the following are homomorphisms/isomorphisms of binary structures? Explain.
	  \begin{enumerate}
	    ...
	    \item\label{exs:easyhomoexdet} %\makebox[210pt][l]{$\phi:(R_4,\circ)\to (\Z_8,+_8)$, \ $\phi(\rho_k)=2k$\hfill (h)\ }
