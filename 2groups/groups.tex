...
\footnotetext{%
	Associativity (Definition \ref{defn:assoc}) does not care whether $x(yz)=(xy)z$ lives in any given subset!%
}
...
\begin{exercises}
	...
	\begin{enumerate}
	  ...
	  \item The set $\Z_n^\times$ denotes the \emph{units} in $\Z_n$, those elements which are relatively prime to $n$:
	  \[
	  ...
	  \begin{enumerate}
	    ...
	    \item Construct the Cayley tables for $\Z_8^\times$ and $\Z_9^\times$. What is the order of each group?
...
\begin{examples}{}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  \item\label{ex:hexsubgroup} Consider a \textcolor{blue}{regular hexagon}, inside which have been drawn two \textcolor{red}{equilateral triangles}. Every symmetry of either triangle is also a symmetry the hexagon. We conclude:
	 	\begin{center}
...
\begin{example}{}{z3r3iso}
	Compare the group $\Z_3$ to the rotation group $R_3$ of an equilateral triangle. Their Cayley tables look almost identical, particularly if we write $\rho_0$ for the identity in $R_3$. To a mathematician, the groups have the same \emph{structure}; they are merely labelled differently.\par
	\begin{minipage}[t]{0.63\linewidth}\vspace{-5pt}
		\emph{Relabelling} means defining an \emph{invertible function} $\mu:\Z_3\to R_3$: here $\mu(x)=\rho_1^x$ (that is $\mu(0)=\rho_0$, $\mu(1)=\rho_1$, $\mu(2)=\rho_2$).\smallbreak
		Since the Cayley tables describe all possible interactions between the elements of each group, it is clear that $\mu$ satisfies
		\[
			\forall x,y\in\Z_3,\ \ \mu(x+_3y)=\mu(x)\circ\mu(y)
		\]
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.36\linewidth}\vspace{-8pt}
		...
		\end{tabular}
	\end{minipage}\bigbreak
	This is a critical formula. To see why, suppose we are given remainders $x,y\in\Z_3$ and consider two courses of action:
...
\begin{examples}{}{}
	\exstart (\ref{ex:z3r3iso} cont.)\quad 	We have \emph{isomorphic groups} $\Z_3\cong R_3$. Indeed the function $\mu:\Z_3\to R_3$ is an \emph{isomorphism of groups} (or \emph{group isomorphism}).
...
\begin{quote}
	\begin{description}
		\item[\emph{Definition}:] Define $\mu:G\to H$ and, if necessary, verify that it is a function.\footnote{%
			If $G$ is a set of equivalence classes, you will need to check that $\phi$ is \emph{well-defined.} This will become important later.%
			}
...
\begin{exercises}{}
	...
	\begin{enumerate}	  
	  ...
	  \begin{enumerate}
	    ...
	    \item \makebox[210pt][l]{$\phi:(\Z_4,+_4)\to (R_8,\circ)$, \ $\phi(x)=\rho_{2x}$\hfill (h)\ }
	  ...
	  \item In Example \ref{ex:z3r3iso} we saw that $\Z_3\cong R_3$ via the explicit isomorphism $\mu(x)=\rho_1^x$. Find a \emph{second} isomorphism $\psi:\Z_3\to R_3$.
	  ...
	  \]
	  \begin{enumerate}
	    \item Compute the image and kernel of the homomorphism $\phi:(\R^\times,\cdot)\to(\R^\times,\cdot)$ defined by $\phi(x)=x^2$.
	  ...
	  \item (Recall Exercise \ref*{sec:geomgroups}.\ref{exs:subgpmatrix} and Footnote \ref{fn:o2matrices}) \ Prove that $S^1\cong\rSO_2(\R)$ via an isomorphism
		...
		\begin{enumerate}
		  \item Up to isomorphism, explain why there is a unique group of order 3.\par
