...
\begin{examples*}{\ref{ex:actioneasy} cont}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \item The action of a group on itself by left multiplication is both faithful and transitive. Conjugation is more complex: in this context, the stabilizer of $x\in G$ is called its \emph{centralizer} $C(x)=\Stab(x)$.
...
\subsection{Orbits \& Burnside's Formula}\label{sec:burnside}
...
\begin{examples}{}{burnsimple}
	\exstart Let $\sigma=(1\,4)(2\,7\,3)\in S_7$. Consider $X=\{1,2,3,4,5,6,7\}$ under the action of the cyclic group $G=\ip{\sigma}$. The orbits are precisely the disjoint cycles: $\{1,4\}$, $\{2,3,7\}$, $\{5\}$, $\{6\}$. Observe that $G$ has six elements:
	\[
		e,\quad 
	...
	\end{gather*}
	\begin{enumerate}\setcounter{enumi}{1}
	  \item\label{exs:burnsimple2} In Exercise \ref*{sec:conj}.\ref{exs:centralizerSn} we computed several centralizers (stabilizers under conjugation). Notice what the Lemma says about the size of the orbit of the 3-cycle $\sigma=(1\,2\,3)$ under conjugation:
	  \[
	  	\nm{S_3(\sigma)}=\frac{\nm{S_3}}{\nm{C(\sigma)}}=\frac{3!}3=2
	  \]
...
\begin{example*}{\ref{ex:burnsimple}.1 cont}{}
	Here is the data when $G=\ip\sigma=\ip{(1\,4)(2\,7\,3)}$ acts on $X=\{1,2,3,4,5,6,7\}$:
	\[
		\begin{array}[t]{c|c}
			x\in X&\Stab(x)\\\hline
...
\begin{exercises}{}{}
	...
	\begin{enumerate}
	  ...
	  \item\label{exs:a5setup} Revisit Exercise \ref*{sec:conj}.\ref{exs:centralizerSn} and Example \ref*{ex:burnsimple}.\ref{exs:burnsimple2}. Prove that there are two distinct conjugacy classes of 5-cycles in $A_5$. Provide an example of two 5-cycles which are not conjugate in $A_5$.
...
\begin{examples}{}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}\itemsep2pt
		\item $G=\ip{2}(\cong\Z_9)$ is a 3-subgroup of $\Z_{18}$ (this last is not a 3-group).
		\item $\Z_3\times\ip{2}$ is a 3-subgroup of $\Z_3\times\Z_6$.
...
\begin{exercises}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  (\emph{Hints: one direction uses Lagrange's Theorem, the other Cauchy})
		...
		\item The alternating group $A_5$ has order $60=2^2\cdot 3\cdot 5$ and comprises the identity, all 3-cycles, 5-cycles, and 2,2-cycles in $S_5$.
		\begin{enumerate}
		  \item Describe all the Sylow 2-, 3-, and 5-subgroups of $A_5$ and verify that the number of each is in line with the 3\rd{} Theorem.
		  \item Extending Exercise \ref*{sec:burnside}.\ref{exs:a5setup}, find the sizes of all conjugacy classes in $A_5$. Hence prove that $A_5$ is a \emph{simple group}: if $N$ is a normal subgroup of $A_5$, then $N=\{e\}$ or $A_5$.\par
		  (\textsf{Simple groups are very important and have applications throughout mathematics. Their full classification was completed in 2004---an enormous undertaking: look it up\ldots})
		\end{enumerate}
